...
import {
  ...
  FaChartLine,
  FaCheck,
   FaChevronDown, FaChevronUp, FaCheckCircle, FaExclamationCircle, FaTimesCircle
} from "react-icons/fa";
import { InView } from "react-intersection-observer";
// import AnalysisSectiona from "./trial";
// import IterationSection from "./iteration";
import { THEME_COLORS, MY_IDEAS, OTHERS_IDEAS, CHECKLIST_ITEMS, RECOMMENDATIONS, VET_IMAGES, MEMBERSHIP_IMAGES, RESEARCH_DATA, PHASES, META, OBJECTIVES, DECISION_THRESHOLDS, EVALUATION_CRITERIA, QUESTIONS, PRIORITIES } from "./petcoData";
  ...
  return (
    <section className="w-full bg-white">
      ...
      <InView
        ...
        style={{ backgroundColor: THEME_COLORS.ex4 }}
      >
        <div className="pb-20 text-[#f0f0f0]">
          <motion.h2
                ...
                {RECOMMENDATIONS[0].risks.map((risk, idx) => (
                  <div key={idx} className="mb-6">
                    <p className="font-semibold mb-2 flex">
                      <div className="bg-white mr-2 text-gray-800 rounded-[0.75rem] w-[1.5rem] h-[1.5rem] flex items-center justify-center">{idx + 1}</div> <div className="flex-[16]">{risk.title}</div>
                    </p>
            ...
            <div className="flex h-full">
              ...
              <div className="h-full">
                <div className="border-b-2 pb-2 border-l-2 pl-2">
                  <div className="grid md:grid-cols-3 gap-1 text-gray-800">
                    <div
                      className="h-[120px] w-[120px] flex justify-center items-center p-2 font-bold"
                      style={{ background: "#977f2d" }}
                    >
                      &nbsp;
                    </div>
                    <div
            ...
            <div className="justify-center bg-gray/70 w-[27%]">
              ...
              <div className="">
                {RECOMMENDATIONS[1].risks.map((risk, idx) => (
                  <div key={idx} className="mb-6">
                    <p className="font-semibold mb-2 flex">
                      <div className="bg-white mr-2 text-gray-800 rounded-[0.75rem] w-[1.5rem] h-[1.5rem] flex items-center justify-center">{idx + 4}</div> <div className="flex-[16]">{risk.title}</div>
                    </p>
                    <p className="">Mitigation: {risk.mitigation}</p>
                  </div>
                ))}
      ...
      <div className="mx-auto px-8 py-20">
        ...
        <div className="relative items-center flex flex-col">
          ...
          <div className="relative w-[60%]">
            <img
              src="/assets/img/projects/petco/petco4.png"
              alt="Analysis"
...
const ResearchSection = () => {
  ...
  return (
    ...
    style={{ background: THEME_COLORS.ex4 }}>
      <motion.h2
        className="text-[8rem] md:text-6xl font-bold mb-6 pt-10 ml-10 mr-[7.5rem] border-b-2 pb-2 text-white"
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
      >
...
const StepsSection = () => {
  return (
    <div
      className="m-0"
      style={{
        backgroundColor: THEME_COLORS.ex5,
...
const AnalysisSection = () => {
  ...
  return (
    <div className=""
    style={{ background: "url('/assets/img/projects/petco/petco4.jpg')", backgroundSize: 'cover' }}>
      <div className="px-12 py-8 mx-auto backdrop-blur-sm bg-[#0e2134]/70">
        {/* Header */}
        <div className="mb-12">
          {/* <div className="inline-block bg-white px-6 py-3 rounded-lg shadow-sm mb-4">
            <span className="text-4xl font-bold text-slate-800">05.</span>
          </div> */}
          <motion.h2
        className="text-[8rem] md:text-6xl font-bold mb-6 pt-10 mr-[7.5rem] border-b-2 pb-2 text-white"
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
      >
        Analysis
          ...
          <p className="text-xl text-white leading-relaxed ">
            Once we selected the strongest hypothesis, we designed an analytical plan to answer 
        ...
        <div className="mb-16">
          <h2 className="text-2xl font-bold text-[#f0f0f0] mb-6 flex items-center">
            <span className="w-2 h-8 bg-[#5A90AE] mr-4 rounded"></span>
          ...
          <div className="space-y-3">
            {QUESTIONS.map((question, index) => (
              <div
                key={index}
                className="bg-white rounded-lg p-5 shadow-sm hover:shadow-md transition-all duration-200 border-l-4 border-[#5A90AE]"
              >
                <div className="flex items-start">
                  <span className="text-[#5A90AE] font-bold mr-4 text-lg">{String(index + 1).padStart(2, '0')}.</span>
                  <p className="text-slate-700 text-lg flex-1">{question}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mb-16">
          ...
          <p className="text-white mb-8 text-lg">
            Given limited time and public data constraints, we prioritized analyses that directly 
            affected go/no-go decisions.
          </p>
          <div className="grid md:grid-cols-3 gap-6">
            {PRIORITIES.map((priority, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`${priority.bgColor} rounded-xl p-6 border-2 ${priority.borderColor} 
                    transition-all duration-300 cursor-pointer hover:scale-105 hover:shadow-lg`}
                  onClick={() => toggleCard(index)}
                >
                  <div className="flex items-start justify-between mb-4">
                    <Icon className={`${priority.color} w-8 h-8`} />
                    {isExpanded ? (
                  ...
                  <h3 className={`${priority.extraColor} font-bold mb-2 text-lg`}>
                    {priority.category}
                  </h3>
                  <p className={`${priority.color} italic text-sm`}>
                    {priority.description}
                  </p>
                  
                  {isExpanded && (
                    <div className="mt-4 pt-4 border-t border-slate-200">
                      <p className={`${priority.extraColor}  text-sm`}>
                        {index === 0 && "Focused on analyses with high certainty that would definitively impact strategic decisions."}
                        {index === 1 && "Used alternative data sources and estimates when perfect data wasn't available."}
                        {index === 2 && "Clearly defined boundaries to maintain focus and timeline."}
                      </p>
                    </div>
                  )}
                </div>
        ...
        {/* <div className="mb-16">
          <h2 className="text-2xl font-bold text-slate-900 mb-6 flex items-center">
            <span className="w-2 h-8 bg-emerald-600 mr-4 rounded"></span>
            What insights actually changed our thinking?
          </h2>
          <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-2xl p-8 border-2 border-emerald-200 shadow-lg">
            <div className="flex items-start space-x-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xl text-slate-700 font-medium">
                  Convenience, not price, was the dominant driver of missed care
                </p>
              </div>
            </div>
          </div>
        ...
        {/* <div className="bg-white rounded-2xl p-8 shadow-lg">
          ...
          <div className="space-y-4">
            <div className="flex items-center space-x-4">
              <div className="bg-blue-100 rounded-full px-4 py-2 text-blue-700 font-semibold">1</div>
              <div className="flex-1 bg-slate-50 rounded-lg p-4">
                <p className="text-slate-700">Insights shaped the storyline</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-4">
              <div className="bg-purple-100 rounded-full px-4 py-2 text-purple-700 font-semibold">2</div>
              <div className="flex-1 bg-slate-50 rounded-lg p-4">
                <p className="text-slate-700">Stress-tested in red team review</p>
              </div>
            </div>
            <div className="flex items-center space-x-4 pl-8">
              <div className="text-slate-400">↓</div>
            ...
            <div className="flex items-center space-x-4">
              <div className="bg-emerald-100 rounded-full px-4 py-2 text-emerald-700 font-semibold">3</div>
              <div className="flex-1 bg-slate-50 rounded-lg p-4">
                <p className="text-slate-700">Validated in final presentation</p>
              </div>
            </div>
          </div>
        </div> */}
